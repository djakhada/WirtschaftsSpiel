...
public class GameInterface extends JFrame{
	...
	//Game Variablen Die wir immer wieder zum Testen benutzen
	...
	//Alle Labels, Texrfelder, CheckBoxen usw.
	...
	//Extra Labels, Texrfelder, CheckBoxen usw.
	...
	//Die Updaten Funktion die alle werte Vom server bekommt und noch mal aktualisiert
	public void Updaten(){
		System.out.println("Client: GameInterface Updaten"); 	//Wir geben Viele sachen in der konsole aus um sicherzugehen das alles übertragen wird
		geldKapital = VF.getGeldKapital();						//setze geldKapital gleich mit dem Wert aus dem VerbindungsFenster (Andere Klasse)
		maxAbbau = VF.getMaxAbbau()+Forschungsabbau;			//setze maxAbbau    gleich mit dem Wert aus dem VerbindungsFenster (Andere Klasse)
		freeLager = VF.getLagerMax()-VF.getLager();				//setze freeLager   gleich mit dem Wert aus dem VerbindungsFenster (Andere Klasse)
		maxLager = VF.getLagerMax()+Forschungslager;			//setze maxLager    gleich mit dem Wert aus dem VerbindungsFenster (Andere Klasse)
		Lager = VF.getLager();									//setze Lager       gleich mit dem Wert aus dem VerbindungsFenster (Andere Klasse)
		maxKredit = VF.getMaxKredit();							//setze maxKredit   gleich mit dem Wert aus dem VerbindungsFenster (Andere Klasse)

		//Gucken ob Kredit verfügbar ist
		if(VF.getKreditLaufzeit()>0) {				//prüfe ob die Keditlaufzeit größer als 0 ist
				btnKredit.setEnabled(false);		//wenn das so ist kann man keinen neuen nehmen
			}
		else {										//wenn das nicht so ist Kann man einen neuen nehmen
				btnKredit.setEnabled(true);
				chckbxlaufzeit1.setEnabled(true);
				chckbxlaufzeit2.setEnabled(true);
			}

		lblGeldKapital.setText("Geldkapital: "+geldKapital+"€");															//setzt Labes mit ihren werten und text fest
	...
	/**
	 * Nur ECLIPSE relevannt um das Fenster alleine zu starten
	 */
	...
	public GameInterface(VerbindungsFenster VF) {
		...
		btnRunde.setEnabled(true);
		btnRunde.addActionListener(new ActionListener() {								//Wenn der Knopf Runde Gedrückt wurde dann
			public void actionPerformed(ActionEvent e) {
				int LagerMenge = Integer.parseInt(txtLagern.getText())-Integer.parseInt(txtLagerVerkauf.getText());				//Hole diese werte weiter
				int VerkaufMenge = Integer.parseInt(txtVerkaufen.getText())+Integer.parseInt(txtLagerVerkauf.getText());
																																//Überprüfe Ob mann wirklich die Runde beenden möchte
			    ...
			    if (dialogResult==JOptionPane.YES_OPTION){
			    	btnRunde.setEnabled(false);																					//wenn es so ist gebe die werte an das Verbindungsfenster
			    	VF.setAbbaumenge(Integer.parseInt(txtLagern.getText())+Integer.parseInt(txtVerkaufen.getText()));			//wenn es nicht so ist mache nichts so dass der spieler wieder sacheen ändern kann
		...
		sliderLagern.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				int Abbaumenge = Integer.parseInt(txtAbbaumenge.getText());   			//Dieser Ganze block rechnet nur die Prozente für die slider und deren text felder aus um sie richtig auszurichten
				int neuValue = 100;
				if(Abbaumenge<=freeLager) {
					neuValue = Abbaumenge * sliderLagern.getValue()/100;				//Rechnet die prozent aus
					txtLagern.setText(neuValue+"");										//setzt den wert in das Textfeld
					sliderVerkaufen.setValue(100-sliderLagern.getValue());
				}else {
					neuValue = freeLager * sliderLagern.getValue()/100;					//Rechnet die prozent aus
					int Differenz = Abbaumenge - neuValue;
					txtLagern.setText(neuValue+"");										//setzt den wert in das Textfeld
					sliderVerkaufen.setValue(100-sliderLagern.getValue());				//Rechnet die prozent aus
					txtVerkaufen.setText(Differenz+"");									//setzt den wert in das Textfeld
				}
		...
		sliderVerkaufen.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				int neuValue = Integer.parseInt(txtAbbaumenge.getText()) * sliderVerkaufen.getValue()/100;			//Rechnet die prozent aus
				txtVerkaufen.setText(neuValue+"");										//setzt den wert in das Textfeld
				sliderLagern.setValue(100-sliderVerkaufen.getValue());					//setzt den slider neu
			}
		});
		...
		panel.add(lblAbbaukosten);
																											//Dieser Block ist für die Krediete zuständig
		btnKredit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				kredit =Integer.parseInt(txtKredit.getText());
				if (chckbxlaufzeit1.isSelected()&&!chckbxlaufzeit2.isSelected())	{						// überprüft ob beide laufzeiten ausgewählt sind sind
					laufzeit=3;
					chckbxlaufzeit1.setEnabled(false);														//disabled die chackboxen
					chckbxlaufzeit2.setEnabled(false);
					btnKredit.setEnabled(false);
					VF.setGenommenerKredit(kredit);															//setzt die Laufzeit und die simme des genommenden kedites fest
					VF.setKreditLaufzeit(laufzeit);
				}

				else if (chckbxlaufzeit2.isSelected()&&!chckbxlaufzeit1.isSelected())	{					// überprüft ob beide laufzeiten ausgewählt sind
					laufzeit=6;
					chckbxlaufzeit2.setEnabled(false);														//disabled die chackboxen
					...
					JOptionPane.showMessageDialog(null,"Fehler bei laufzeitwahl", "fatal error", JOptionPane.ERROR_MESSAGE);				//fehler massage
				}
			}
		});

		//Erstellen des Kedit Fensters
		...
		panel_1.add(lblSbagger);
														//setze Die Forschungen die man erst Freischalten muss auf nicht wählbar
		chckbxlager2.setEnabled(false);
		chckbxlager3.setEnabled(false);
		chckbxarbeiter2.setEnabled(false);
		chckbxarbeiter3.setEnabled(false);
		...
		chckbxstreikumsatzausfall.setEnabled(false);
												//Überprüfe welche Forschung ausgewählt wurde und setze diese auf nicht mehr wählbar und gibt weite frei
		...
		JLabel lblForschungBild = new JLabel(iconforschung);			//setzt das Forschungsbild
